...
const SidebarJobs: React.FC<Props> = ({ jobs }) => {
  return (
    <div className={styles.sidebarCard}>
      ...
      <div className={styles.sidebarSearchRow}>
        ...
        />
        {/* filter dropdown future */}
      </div>

      <div className={styles.sidebarJobsList}>
        {jobs.length === 0 ? (
          <div className={styles.sidebarEmptyState}>
            <div className={styles.sidebarEmptyEmoji}>
              🐝
            </div>
          ...
          jobs.map((job) => (
            ...
            >
              <div className={styles.sidebarJobTitle}>
                {job.title}
              </div>
              <div className={styles.sidebarJobCustomer}>
                {job.customer}
              </div>
              {job.location && (
                <div
                  className={
                    styles.sidebarJobLocation
                  }
                >
                  {job.location}
                </div>
