...
exports.getFavoriteItems = exports.deleteServiceCatalogItem = exports.updateServiceCatalogItem = exports.createServiceCatalogItem = exports.getServiceCatalogItems = exports.deleteServiceCatalog = exports.updateServiceCatalog = exports.createServiceCatalog = exports.getServiceCatalogs = void 0;
...
exports.deleteServiceCatalog = deleteServiceCatalog;
/* ===============================
   SERVICE CATALOG ITEMS
================================ */
const getServiceCatalogItems = async (req, res) => {
    const db = req.db;
    const serviceCatalogId = Number(req.params.id);
    try {
        const result = await db.query(`
...
exports.getServiceCatalogItems = getServiceCatalogItems;
const createServiceCatalogItem = async (req, res) => {
    const db = req.db;
    const serviceCatalogId = Number(req.params.id);
    const { name, description, unit, cost_price, sell_price, tax_rate, is_favorite = false, } = req.body || {};
    if (!name || String(name).trim() === "") {
        return res.status(400).json({ error: "Name is required" });
    }
    try {
...
const updateServiceCatalogItem = async (req, res) => {
    const db = req.db;
    const serviceCatalogId = Number(req.params.id);
    const itemId = Number(req.params.itemId);
    const { name, description, unit, cost_price, sell_price, tax_rate, is_favorite, } = req.body || {};
    try {
        const result = await db.query(`
...
const deleteServiceCatalogItem = async (req, res) => {
    const db = req.db;
    const serviceCatalogId = Number(req.params.id);
    const itemId = Number(req.params.itemId);
    try {
