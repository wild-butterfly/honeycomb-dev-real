...
const generateInvoicePdf = async (req, res) => {
    try {
        const { id } = req.params;
        // Fetch invoice data
        const invoiceResult = await db_1.pool.query(`SELECT * FROM public.invoices WHERE id = $1`, [id]);
        ...
        const lineItemsResult = await db_1.pool.query(`SELECT * FROM public.invoice_line_items WHERE invoice_id = $1 ORDER BY id`, [id]);
        ...
        if (invoice.company_id) {
            ...
            const templateResult = await db_1.pool.query(`SELECT * FROM public.invoice_templates WHERE company_id = $1 AND is_default = true ORDER BY created_at DESC LIMIT 1`, [invoice.company_id]);
            ...
            const companyResult = await db_1.pool.query(`SELECT id, name, logo_url FROM public.companies WHERE id = $1`, [invoice.company_id]);
            ...
            const settingsResult = await db_1.pool.query(`SELECT * FROM public.invoice_settings WHERE company_id = $1`, [invoice.company_id]);
        ...
        if (companyCity) {
            const cityState = [companyCity, settings?.company_state]
                .filter(Boolean)
        ...
        if (invoice.customer_id) {
            try {
                const customerResult = await db_1.pool.query(`SELECT * FROM public.customers WHERE id = $1`, [invoice.customer_id]);
        ...
        if (invoice.job_id) {
            try {
                const jobResult = await db_1.pool.query(`SELECT * FROM public.jobs WHERE id = $1`, [invoice.job_id]);
        ...
        doc.font("Helvetica").text(invoice.created_at ? new Date(invoice.created_at).toLocaleDateString() : "N/A");
        metaY += 14;
        // Due date
        doc.font("Helvetica-Bold").text("Due Date: ", col3X, metaY, { continued: true });
        doc.font("Helvetica").text(invoice.due_date ? new Date(invoice.due_date).toLocaleDateString() : "N/A");
        metaY += 14;
        ...
        if (settings && settings.gst_number) {
            ...
            doc.font("Helvetica").text(settings.gst_number);
            metaY += 14;
        }
        ...
        if (useTemplateSections) {
            ...
            template.parsedSections.forEach((section, sectionIndex) => {
                // Section header
                doc
                    .fontSize(12)
                    .font("Helvetica-Bold")
        ...
        doc
            ...
            .text(`$${(invoice.subtotal || 0).toFixed(2)}`, totalsX + 100, yPosition, { align: "right", width: 70 });
        yPosition += 20;
        doc
            .text("Tax:", totalsX, yPosition)
            .text(`$${(invoice.tax_amount || 0).toFixed(2)}`, totalsX + 100, yPosition, { align: "right", width: 70 });
        yPosition += 20;
        // Total box
        doc
        ...
        doc
            ...
            .text(`$${(invoice.total_with_tax || 0).toFixed(2)}`, totalsX + 100, yPosition + 6, { align: "right", width: 70 });
        yPosition += 40;
        // ===== PAYMENT TERMS & NOTES =====
