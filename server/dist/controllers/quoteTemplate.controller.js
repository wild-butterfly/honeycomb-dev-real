...
exports.previewQuoteTemplatePdf = exports.deleteQuoteTemplate = exports.updateQuoteTemplate = exports.createQuoteTemplate = exports.getQuoteTemplate = exports.getCompanyQuoteTemplates = void 0;
...
const ensureQuoteTemplateStyleColumns = async () => {
    if (quoteTemplateStyleColumnsEnsured)
        return;
...
const getQuoteTemplateColumns = async () => {
...
const getCompanyQuoteTemplates = async (req, res) => {
    const { companyId } = req.params;
    try {
        await ensureQuoteTemplateStyleColumns();
        const result = await db_1.pool.query(`SELECT * FROM quote_templates WHERE company_id = $1 ORDER BY is_default DESC, created_at DESC`, [companyId]);
...
exports.getCompanyQuoteTemplates = getCompanyQuoteTemplates;
const getQuoteTemplate = async (req, res) => {
    const { id } = req.params;
    try {
        await ensureQuoteTemplateStyleColumns();
        const result = await db_1.pool.query(`SELECT * FROM quote_templates WHERE id = $1`, [id]);
...
const createQuoteTemplate = async (req, res) => {
    ...
    if (Array.isArray(body.sections))
        ...
        try {
            sectionsToSave = JSON.parse(body.sections);
        }
        catch {
            sectionsToSave = [];
        }
        ...
        sectionsToSave = [];
    if (body.is_default) {
        await db_1.pool.query(`UPDATE quote_templates SET is_default = false WHERE company_id = $1`, [body.company_id]);
    }
    try {
        await ensureQuoteTemplateStyleColumns();
        ...
        addOptionalColumn("table_header_background_color", body.table_header_background_color || body.main_color || "#fbbf24");
        addOptionalColumn("table_header_gradient_color", body.table_header_gradient_color || body.main_color || "#f59e0b");
        ...
        addOptionalColumn("description_border_color", body.description_border_color || body.border_color || body.main_color || "#fbbf24");
        ...
        addOptionalColumn("show_company_logo", body.show_company_logo !== undefined ? body.show_company_logo : true);
        const placeholders = values.map((_, idx) => `$${idx + 1}`).join(",");
        const result = await db_1.pool.query(`INSERT INTO quote_templates (${insertColumns.join(",")})
       VALUES (${placeholders})
       RETURNING *`, values);
...
const updateQuoteTemplate = async (req, res) => {
    ...
    if (Array.isArray(body.sections))
        ...
        sectionsToSave = [];
    await ensureQuoteTemplateStyleColumns();
    const templateResult = await db_1.pool.query(`SELECT * FROM quote_templates WHERE id = $1`, [id]);
    if (templateResult.rows.length === 0)
        return res.status(404).json({ error: "Quote template not found" });
    const existing = templateResult.rows[0];
    const company_id = existing.company_id;
    if (body.is_default) {
        await db_1.pool.query(`UPDATE quote_templates SET is_default = false WHERE company_id = $1 AND id != $2`, [company_id, id]);
    }
    try {
        const columns = await getQuoteTemplateColumns();
        ...
        let mergedSections = sectionsToSave;
        ...
        addSet("table_header_background_color", merged.table_header_background_color || merged.main_color || "#fbbf24");
        addSet("table_header_gradient_color", merged.table_header_gradient_color || merged.main_color || "#f59e0b");
        ...
        addSet("description_border_color", merged.description_border_color || merged.border_color || merged.main_color || "#fbbf24");
        ...
        const result = await db_1.pool.query(`UPDATE quote_templates SET
        ${setParts.join(", ")}, updated_at = CURRENT_TIMESTAMP
       WHERE id = ${idParam} RETURNING *`, values);
        let template = result.rows[0];
        if (template.sections && typeof template.sections === 'string') {
            try {
                template.sections = JSON.parse(template.sections);
...
const deleteQuoteTemplate = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db_1.pool.query(`DELETE FROM quote_templates WHERE id = $1 RETURNING *`, [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: "Quote template not found" });
        }
...
const previewQuoteTemplatePdf = async (req, res) => {
    try {
        ...
        const previewLineItems = sectionItems.length > 0
            ? sectionItems
            : [
        ...
        const previewSubtotal = previewLineItems.reduce((sum, item) => sum + toNumber(item.total, 0), 0);
...
exports.previewQuoteTemplatePdf = previewQuoteTemplatePdf;
