...
(async () => {
    try {
        ...
        const source = await db_1.pool.query('SELECT * FROM invoice_templates WHERE id = 5 AND company_id = 1');
        ...
        await db_1.pool.query('UPDATE invoice_templates SET is_default = false WHERE company_id = 2');
        // Copy template to Company 2
        const insert = await db_1.pool.query(`INSERT INTO invoice_templates (
        ...
        console.log('\n=== VERIFICATION ===');
        const verify = await db_1.pool.query('SELECT id, name, is_default, table_header_background_color FROM invoice_templates WHERE company_id = 2');
