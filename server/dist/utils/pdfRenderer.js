...
const renderInvoicePdf = async ({ invoice, lineItems, template, settings, company, customer, job, res, }) => {
    ...
    if (template?.show_line_items !== false) {
        ...
        const headerFill = template?.table_header_style === "gradient"
            ...
            : tableHeaderBg;
        doc.rect(margin, y, contentWidth, 22).fill(headerFill);
        ...
        lineItems.forEach((item, idx) => {
            ...
            const rawDescription = (item.description || "").replace(/auto-generated from completed assignment/gi, "");
            ...
            let cellX = margin;
            columnWidths.forEach((column) => {
                const cellHeight = column.key === "description"
                    ? descHeight
                    : column.key === "quantity"
                        ? qtyHeight
                        : column.key === "price"
